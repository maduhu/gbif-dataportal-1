...
begin
	...
	_loop: loop
		 ...
		 */
		call getTaxonConceptIdOfSpecies(_kingdom, _phylum, _class, _order, _family, _genus, _species, _taxon_concept_id);
...
/**
 * This will get the taxon_concept id of the given classification of species
 */
DROP procedure if EXISTS getTaxonConceptIdOfSpecies;
delimiter |
create procedure getTaxonConceptIdOfSpecies(IN _kingdom_name CHAR(255), IN _phylum_name CHAR(255), IN _class_name CHAR(255), IN _order_name CHAR(255), IN _family_name CHAR(255), IN _genus_name CHAR(255), IN _species_name CHAR(255), INOUT _species_id INT)
begin
	declare _done BOOLEAN DEFAULT false;
    declare _cur1 cursor for 
    	select sc.id
	    from taxon_concept sc inner join taxon_name sn on sc.taxon_name_id = sn.id
	    	inner join taxon_concept gc on sc.genus_concept_id = gc.id inner join taxon_name gn on gc.taxon_name_id = gn.id
	    	inner join taxon_concept fc on sc.family_concept_id = fc.id inner join taxon_name fn on fc.taxon_name_id = fn.id
	    	inner join taxon_concept oc on sc.order_concept_id = oc.id inner join taxon_name orn on oc.taxon_name_id = orn.id
	    	inner join taxon_concept cc on sc.class_concept_id = cc.id inner join taxon_name cn on cc.taxon_name_id = cn.id
	    	inner join taxon_concept pc on sc.phylum_concept_id = pc.id inner join taxon_name pn on pc.taxon_name_id = pn.id
	    	inner join taxon_concept kc on sc.kingdom_concept_id = kc.id inner join taxon_name kn on kc.taxon_name_id = kn.id
		where sc.data_resource_id = @_data_resource_id and 
			sc.rank = 7000 and
			kn.canonical = _kingdom_name and
			pn.canonical = _phylum_name and
			cn.canonical = _class_name and
			orn.canonical = _order_name and
			gn.canonical = _genus_name and
			sn.specific_epithet = _species_name;
    declare continue handler for not found SET _done = true;
	open _cur1;
	_loop: loop
		fetch _cur1 INTO _species_id;
		close _cur1;
		leave _loop;
	end loop _loop;
end;
