 ...
 * @author trobertson
 */
public class InstitutionCodeDAOImpl extends JdbcDaoSupport implements
		InstitutionCodeDAO {
	/**
	 * The create sql
	 */
	protected static final String CREATE_SQL = "insert into institution_code(" +
												"code) " +
												"values (?)";
	...
	"from institution_code where code=?";
	...
	/**
	 * Gets the id of the InstitutionCode representing the string supplied
	...
	public Long getIdForIdentifier(final String identifier) {
		...
		.query(InstitutionCodeDAOImpl.QUERY_BY_IDENTIFIER_SQL,
			new Object[] {identifier},
			new RowMapperResultSetExtractor(longRowMapper));
		if (results.size() == 0) {
			return null;
		} else if (results.size()>1) {
			logger.warn("Found multiple InstitutionCodes with identifier: " + identifier);
		}
		return results.get(0);
	}
	
	/**
	 * @see org.gbif.portal.dao.InstitutionCodeDAO#createIfNotExists(java.lang.String)
	 */
			...
			getJdbcTemplate().update(
				new PreparedStatementCreator() {
					public PreparedStatement createPreparedStatement(Connection conn) throws SQLException {
						   PreparedStatement ps = conn.prepareStatement(InstitutionCodeDAOImpl.CREATE_SQL);
