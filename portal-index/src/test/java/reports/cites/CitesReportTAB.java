...
public class CitesReportTAB {
	...
	protected void run(String filename) throws IOException {
		...
		// remove things that are ready to be removed 
	...
	protected List<String> format(List<String> names) {
		...
		for (String name : names) {
			if (!namesToIgnore.contains(name) &&
					!StringUtils.equals(lastName, name)) {
				//log.info(name);
				results.add(name);
			}
			lastName = name;
