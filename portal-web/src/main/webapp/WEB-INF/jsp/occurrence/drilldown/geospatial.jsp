...
<fieldset>
<p>
	<label for="continent"><spring:message code="occurrence.record.continent"/>:</label> 
	<c:choose>
		<c:when test="${rawOccurrenceRecord.continentOrOcean!=null}">
			${rawOccurrenceRecord.continentOrOcean}
			<c:if test="${not empty country.continentCode}">	
				<c:set var="contLink"><spring:message code="continent.${country.continentCode}" text=""/></c:set>
				<c:if test="${rawOccurrenceRecord.continentOrOcean != contLink}">
					<spring:message code="occurrence.record.interpreted.as" arguments="${contLink}"/>
				</c:if>
			</c:if>
		</c:when>
		<c:when test="${not empty country && not empty country.continentCode}">
			<spring:message code="continent.${country.continentCode}" text=""/> <spring:message code="occurrence.record.inferred.from.country"/>
		</c:when>
	</c:choose>		
</p>
<p>	
