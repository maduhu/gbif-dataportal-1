...
>
  ...
  <display:column titleKey="occurrence.search.results.country">
		<c:if test="${not empty occurrenceRecord.isoCountryCode}">
			<spring:message code="country.${occurrenceRecord.isoCountryCode}"/>
			<gbiftag:isCountryInferred issuesBit="${occurrenceRecord.otherIssue}"/>
			<c:if test="${countryInferred}">
				<br/><span style='font-size: 0.7em;'><spring:message code="occurrence.record.inferred.from.coordinates"/></span>
			</c:if>
		</c:if>			
  </display:column>
