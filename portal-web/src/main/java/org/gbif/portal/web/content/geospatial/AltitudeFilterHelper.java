...
/**
 * A Filter Helper for the Altitude Filter.
 * 
 * @author davejmartin
 */
public class AltitudeFilterHelper implements FilterHelper {
	...
	/**
	 * This will take any altitude and round down to an integer.
	 * 
	...
	public void preProcess(List<PropertyStoreTripletDTO> triplets, HttpServletRequest request,
		...
		List<PropertyStoreTripletDTO> subjectTriplets = QueryHelper.getTripletsBySubject(triplets, "SERVICE.OCCURRENCE.QUERY.SUBJECT.ALTITUDE");
		//convert to centimetres
		for(PropertyStoreTripletDTO triplet: subjectTriplets){
			Float floatValue = (Float) triplet.getObject();
			triplet.setObject(Math.round(floatValue));
		}
	}
}