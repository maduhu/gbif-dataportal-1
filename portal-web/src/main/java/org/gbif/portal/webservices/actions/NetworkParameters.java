...
public class NetworkParameters extends Parameters {
	...
	public static final String NETWORK_SERVICE_NAME ="network";
	
	public static final String NETWORK_NAMESPACE = "http://portal.gbif.org/ws/response/gbif";
	...
	public static Log log = LogFactory.getLog(NetworkParameters.class);
	
	protected NetworkParameters() {
		// Null constructor
	}
	
	public NetworkParameters(Map<String, Object> params, PathMapping pathMapping)
		throws GbifWebServiceException
	{
		...
		try {
			...
			throw new GbifWebServiceException("NetworkParameters exception: " + e);
		}
	}
	...
	public Map<String, Object> getParameterMap(Integer overrideRequestType) {
		...
		if (rt != Action.SCHEMA && rt != Action.STYLESHEET) {
			if (requestType == Action.COUNT || requestType == Action.LIST) {
				...
				if (code != null) map.put(KEY_CODE, code);
			}
