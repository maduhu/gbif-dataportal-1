...
public class WorkflowLauncherJob implements Job {
	...
	public void execute(JobExecutionContext quartzContext) throws JobExecutionException {
		try {
			...
			ApplicationContext context = (ApplicationContext) quartzContext.getScheduler().getContext().get("applicationContext");
			PropertyStore propertyStore = (PropertyStore) context.getBean("indexPropertyStore", PropertyStore.class);
			String propertyStoreWorkflowKey = (String)quartzContext.getJobDetail().getJobDataMap().get(PROPERTY_STORE_KEY_WORKFLOW_KEY);
			List<String> propertyStoreNamespaces = (List<String>)quartzContext.getJobDetail().getJobDataMap().get(PROPERTY_STORE_NAMESPACES_KEY);
			Map<String, Object> seed = (Map<String, Object>)quartzContext.getJobDetail().getJobDataMap().get(SEED_DATA_KEY);
			Processor workflow = (Processor) propertyStore.getProperty(propertyStoreNamespaces, propertyStoreWorkflowKey, Processor.class);
