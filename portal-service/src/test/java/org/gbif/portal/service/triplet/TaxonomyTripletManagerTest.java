...
/**
 * Simple test of the taxonomy filter searching.
 *
 * @author Dave Martin
 */
public class TaxonomyTripletManagerTest extends AbstractServiceTest {

	@SuppressWarnings("unchecked")
	public void testDoTripletQuery() throws Exception {
		
		TripletQueryManager manager = (TripletQueryManager) getBean("tripletTaxonomyManager");
		...
		SearchResultsDTO results = manager.doTripletQuery(
				Arrays.asList(new PropertyStoreTripletDTO[]{
						new PropertyStoreTripletDTO("http://gbif.org/portal-service/2006/1.0", 
								"SERVICE.TAXONOMY.QUERY.SUBJECT.CANONICAL",
		...
		List<TaxonConceptDTO> data = results.getResults();
		for (TaxonConceptDTO item : data) 
			logger.debug("retrieved concept:"+item);
	}
