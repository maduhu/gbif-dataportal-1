...
public class DelimitedFileReader {
		...
		try {
			...
			if (headerRow) {
				...
				for (int i = 0; i < columns.size(); i++) {
					columnMap.put((String) columns.get(i), new Integer(i));
				}
			}
		} catch (Exception e) {
	...
	public String get(int column) {
		// \N is a common "null" from databases including a MySQL dump
		return (columns != null && columns.size() > column && !"\\N".equals(columns.get(column))) 
		...
		try {
			...
			if (line != null) {
				...
				while (i1 >= 0 && i1 < line.length()) {
					...
					if (strip != null 
							&& (tokenEnd - tokenStart >= 2 * strip.length())
							&& line.startsWith(strip, tokenStart) 
							&& line.startsWith(strip, tokenEnd - strip.length())) {
