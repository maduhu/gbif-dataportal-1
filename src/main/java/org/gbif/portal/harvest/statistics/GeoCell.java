...
public class GeoCell {
	...
	protected void loop(Connection conn, String parameterName, int cellType, boolean byDecade) {
		...
		Statement stmt = null;
		ResultSet rs = null;
		Map<CellId, Integer> data = new HashMap<CellId, Integer>(10000000);
		try {
			String query = 
				"select " +
			...
			while (rs.next()) {
				...
				if (rs.getObject("cell_id")!=null) {
					...
					if (!byDecade)
						incrementMapCount(data, new CellId(cellId,month,rs.getInt(parameterName)));
					else 
						incrementMapCount(data, new CellId(cellId,decade,rs.getInt(parameterName)));
				}
			...
			if (!byDecade)
				writeToFile("cell-" + parameterName + "Month.txt", cellType, data);
			else 
				writeToFile("cell-" + parameterName + "Decade.txt", cellType, data);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			// release database resources
	...
	class CellId {
		...
		public CellId(int cellId, Integer datePart, int id) {
			this.cellId = cellId;
		...
		public int hashCode() {
			if (datePart == null) 
				return (cellId*100) + id;
			else 
				return (cellId*103) + id + (datePart*717);
		}
	}
	 ...
	 * @param data
	 */
	protected void writeToFile(String fileName, int type, Map<CellId, Integer> data) {
		FileWriter fw = null;
