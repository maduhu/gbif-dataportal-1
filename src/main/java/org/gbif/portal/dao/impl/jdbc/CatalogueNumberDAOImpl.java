 ...
 * @author trobertson
 */
public class CatalogueNumberDAOImpl extends JdbcDaoSupport implements
		CatalogueNumberDAO {
	/**
	 * The create sql
	 */
	protected static final String CREATE_SQL = "insert into catalogue_number(" +
												"code) " +
												"values (?)";
	...
	"from catalogue_number where code=?";
	...
	/**
	 * Gets the id of the CatalogueNumber representing the string supplied
	...
	public Long getIdForIdentifier(final String identifier) {
		...
		.query(CatalogueNumberDAOImpl.QUERY_BY_IDENTIFIER_SQL,
			new Object[] {identifier},
			new RowMapperResultSetExtractor(longRowMapper));
		if (results.size() == 0) {
			return null;
		} else if (results.size()>1) {
			//logger.warn("Found multiple CatalogueNumbers with identifier: " + identifier);
		}
		return results.get(0);
	}
	
	/**
	 * @see org.gbif.portal.dao.CatalogueNumberDAO#createIfNotExists(java.lang.String)
	 */
			...
			getJdbcTemplate().update(
				new PreparedStatementCreator() {
					public PreparedStatement createPreparedStatement(Connection conn) throws SQLException {
						   PreparedStatement ps = conn.prepareStatement(CatalogueNumberDAOImpl.CREATE_SQL);
