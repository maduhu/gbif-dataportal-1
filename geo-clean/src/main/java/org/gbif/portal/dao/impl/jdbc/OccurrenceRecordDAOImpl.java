...
public class OccurrenceRecordDAOImpl extends JdbcDaoSupport implements OccurrenceRecordDAO {
	 ...
	 * The count by nub id
	 */
	protected static final String COUNT_BY_NUB_ID_SQL = 
		"select count(*) from occurrence_record where nub_concept_id=?";

	/**
	 * The count by nub id
	 */
	protected static final String COUNT_BY_NUB_ID_GEOSPATIAL_SQL = 
		"select count(*) from occurrence_record where nub_concept_id=? and cell_id is not null";
	
	/** 
	 * The query by geospatial issue 
	 */
	protected static final String QUERY_BY_GEOSPATIAL_ISSUE = 
		"select ore.id,ore.data_provider_id,ore.data_resource_id," +
		"ore.institution_code_id,ore.collection_code_id,ore.catalogue_number_id," +
		"taxon_concept_id,ore.taxon_name_id,ore.nub_concept_id,ore.iso_country_code," +
		"ore.latitude,ore.longitude,ore.altitude_metres, ore.depth_centimetres," +
	...
	/**
	 * @see org.gbif.portal.dao.OccurrenceRecordDAO#countByNubId(long)
	 */
	public int countByNubId(final long nubId) {
		return (Integer) getJdbcTemplate().query(
				COUNT_BY_NUB_ID_SQL, 
				new Object[]{nubId},
				intRowMapper).get(0);
	}
	 ...
	 * @see org.gbif.portal.dao.OccurrenceRecordDAO#countByNubId(long)
	 */
	public int countByNubIdGeospatial(final long nubId) {
		return (Integer) getJdbcTemplate().query(
				COUNT_BY_NUB_ID_GEOSPATIAL_SQL, 
				new Object[]{nubId},
				intRowMapper).get(0);
	}

	public List<OccurrenceRecord> getByGeospatialIssueNumber(final int geospatialIssue) {
		List<OccurrenceRecord> results = (List<OccurrenceRecord>) getJdbcTemplate()
		.query(OccurrenceRecordDAOImpl.QUERY_BY_GEOSPATIAL_ISSUE,
			new Object[]{geospatialIssue},
			new RowMapperResultSetExtractor(occurrenceRecordRowMapper, 1));
	if (results.size() == 0) {
		return null;
	} 
